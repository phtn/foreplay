...
export const MultiSelect = <T,>({
  ...
  const editableFields = useMemo<EditableField<T>[]>(() => {
    return columnConfigs.flatMap((config) => {
      ...
      const values = selectedRows.map((row) => row[config.accessorKey] as EditableValue | object)
      const definedValues = values.filter(
        (value): value is Exclude<typeof value, null | undefined> => value !== null && value !== undefined
      )
      ...
      const options = resolveFieldOptions(primitiveValues, selectedRows, meta, bulkEditor)
  ...
  const initialDraftValues = useMemo(
    () => Object.fromEntries(editableFields.map((field) => [field.id, field.initialValue])) as Record<string, string>,
    [editableFields]
  )
  ...
  const pendingFieldUpdates = useMemo(
    () => resolvePendingFieldUpdates(editableFields, draftValues, initialDraftValues),
    [draftValues, editableFields, initialDraftValues]
  )
  ...
  return (
    <div className='flex h-full min-h-0 flex-col bg-background/70'>
          ...
          <div className='flex items-start justify-between gap-3'>
            ...
            <div className='flex shrink-0 items-center gap-1.5'>
              <Badge
                ...
                <span className='drop-shadow-xs'>{editableFields.length} fields</span>
              </Badge>
            </div>
          </div>
        </FrameHeader>
                      ...
                      <TableCell
                        ...
                        <div className='flex flex-col gap-1'>
                          ...
                          <div className='flex items-center justify-between font-ios font-light uppercase text-[8px] md:text-[9px] tracking-wider'>
                            <span
                              className={cn('dark:text-emerald-400 text-emerald-600', {
                                'dark:text-indigo-400 text-indigo-600': field.typeLabel === 'select',
                                'dark:text-sky-400 text-sky-600': field.typeLabel === 'number',
                                'dark:text-pink-400 text-pink-500': field.typeLabel === 'boolean'
                              })}>
                              {field.typeLabel}
                            </span>
                            <span
                              className={cn('text-foreground text-[8px] md:text-[9px]', {
                                'text-amber-600 dark:text-orange-300': field.isMixed
                              })}>
        ...
        <FrameFooter className='px-0! pb-0!'>
          <div className='flex flex-wrap items-center justify-between gap-2'>
            <button
              ...
              <span className={cn('flex', { hidden: isCompact })}>{deleteActionLabel}</span>
              <Icon name='trash-delete-fill' className={cn('size-4 hidden', { flex: isCompact })} />
            </button>
      ...
      <Drawer open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <DrawerContent className='border border-border/60 bg-background/95 shadow-2xl'>
          <DrawerHeader className='flex flex-col gap-1 pb-2'>
            <span className='text-base font-semibold tracking-tight'>Review bulk changes</span>
            <p className='text-sm font-normal text-muted-foreground'>
              {pendingFieldUpdates.length} field
              {pendingFieldUpdates.length === 1 ? '' : 's'} will be applied to {selectedRows.length} selected row
          ...
          <div className='gap-4 pb-2'>
            <div className='grid gap-2 sm:grid-cols-2'>
              ...
              <div className='rounded-xl border border-border/60 bg-muted/40 px-4 py-3'>
                ...
                <div className='mt-1 text-lg font-semibold'>{pendingFieldUpdates.length}</div>
              </div>
            </div>
                ...
                {pendingFieldUpdates.map(({ field, displayValue }) => (
                  <div
                    ...
                    <div className='min-w-0'>
                      ...
                      <div className='truncate text-xs text-muted-foreground'>{field.preview}</div>
                    </div>
                    <Icon name='arrow-right' className='size-4 shrink-0 text-muted-foreground' />
                    <div className='min-w-0 text-sm font-medium sm:text-right'>{displayValue}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <p className='text-xs leading-5 text-muted-foreground'>
              Only the fields listed here will be patched onto each selected row.
            </p>
          </div>
      ...
      <Drawer open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
        <DrawerContent className='border border-border/60 bg-background/95 shadow-2xl'>
          <DrawerHeader className='flex flex-col gap-1 pb-2'>
            <span className='text-base font-semibold tracking-tight'>Confirm bulk delete</span>
            <p className='text-sm font-normal text-muted-foreground'>
          ...
          <div className='gap-4 pb-2'>
            <div className='rounded-xl border border-rose-500/20 bg-rose-500/8 px-4 py-3'>
              <div className='flex items-start gap-3'>
                ...
                <div className='min-w-0'>
                  <div className='text-sm font-semibold text-foreground'>This action removes the selected rows.</div>
                  <p className='mt-1 text-xs leading-5 text-muted-foreground'>
                    Review the selection before continuing. This applies to all {selectedRows.length} selected row
                    {selectedRows.length === 1 ? '' : 's'}.
                  </p>
                </div>
              </div>
            ...
            <div className='rounded-xl border border-border/60 bg-muted/40 px-4 py-3'>
              <div className='text-[10px] font-medium uppercase tracking-[0.24em] text-muted-foreground'>Selected</div>
              <div className='mt-1 text-lg font-semibold'>
                {selectedRows.length} row
                {selectedRows.length === 1 ? '' : 's'}
              </div>
...
const normalizeBulkEditorConfig = <T,>(bulkEditor: ColumnMeta<T>['bulkEditor']) => {
...
const resolveFieldOptions = <T,>(
  ...
  if (explicitOptions) {
    const options = typeof explicitOptions === 'function' ? explicitOptions(rows) : explicitOptions
  ...
  const nonNullValues = values.filter((value): value is boolean => typeof value === 'boolean')
  if (nonNullValues.length > 0 && nonNullValues.length === values.filter((value) => value != null).length) {
...
const ensureCurrentOptions = (values: EditableValue[], options: BulkEditorOption[]) => {
...
const getPlaceholder = <T,>(config: ColumnConfig<T>, values: EditableValue[], inputKind: EditableInputKind) => {
  const label = humanizeLabel(String(config.id || config.accessorKey)).toLowerCase()
  if (inputKind === 'select') {
    return `Select ${label}`
  }
...
const getPreviewValue = (values: EditableValue[], options: BulkEditorOption[]) => {
...
const resolvePendingFieldUpdates = <T,>(
  ...
  return editableFields.flatMap((field) => {
    ...
    const normalizedValue = field.inputKind === 'text' ? draftValue : draftValue.trim()
...
const buildUpdatePayload = <T,>(pendingFieldUpdates: PendingFieldUpdate<T>[]) => {
