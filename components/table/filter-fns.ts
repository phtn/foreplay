 ...
 */
export const filterFn = <T>(row: Row<T>, columnId: string, filterValue: unknown): boolean => {
  ...
  if (Array.isArray(filterValue)) {
    ...
    return filterValue.some((fv) => {
      ...
      return (
        normalizedRowTokens.some((token) => token === normalizedFilterValue || token.includes(normalizedFilterValue)) ||
        fv === value
      )
    })
  }
...
export const groupFilter = <T>(row: Row<T>, columnId: string, filterValue: unknown): boolean => {
  const value = row.getValue(columnId)
...
export const globalFilterFn = <T>(row: Row<T>, columnId: string, filterValue: string): boolean => {
  ...
  return rowTokens.some((token) => normalizeText(token).includes(normalizedFilter))
}
