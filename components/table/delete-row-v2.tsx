...
export const DeleteButton = <T,>({ rows, onDelete, idAccessor, disabled = false }: DeleteButtonProps<T>) => {
  ...
  return (
    <Button
      ...
      <Icon
        name={loading ? 'spinner-ring' : 'trash-delete'}
        className={cn('size-4 text-mac-red dark:text-red-400', {
          'opacity-50': disabled || !hasSelection
        })}
      />
      <span className='hidden md:flex text-xs md:pr-3 font-brk uppercase'>Delete</span>
      {selectedCount > 0 && (
