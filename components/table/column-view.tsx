        ...
        render={
          <Button
            ...
            {invisibleColumns.length > 0 ? (
              <span className='min-w-3.5 w-5 grow-0 rounded-sm bg-orange-500 font-okxs font-semibold text-white md:-ml-1 dark:bg-orange-400'>
                {invisibleColumns.length > 99
                  ? '99+'
              ...
              <Icon
                ...
                className='size-4 dark:opacity-80'
              />
            )}
            <span className='hidden font-brk text-sm tracking-wider opacity-80 md:flex'>
              Columns
            </span>
          </Button>
        }>
            ...
            <div className='p-2'>
              {hideableColumns.map((column) => (
                <Menu.CheckboxItem
                  key={column.id}
                  className={cn(
                    'flex h-8 items-center justify-between rounded-sm bg-orange-300 px-3 text-xs text-origin hover:bg-dark-table/10 dark:hover:bg-origin/40 dark:hover:text-orange-300',
                    column.getIsVisible() &&
