    ...
    <Root.Root
      ...
      <Root.List className='relative z-0 flex gap-6'>
        {tabs.map((tab) => (
          <Root.Tab
            key={tab.value}
            className={`flex items-center gap-2 px-2.5 py-1 text-sm font-medium ${
        ...
        <Root.Indicator
          ...
          className='absolute top-1/2 left-0 z-[-1] h-6 w-(--active-tab-width) translate-x-(--active-tab-left) -translate-y-1/2 rounded-md bg-accent/30 dark:bg-slate-500/40 transition-all duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)]'
        />
      </Root.List>
