...
} from '@/lib/resend/webhooks/record'
...
export const record = mutation({
  ...
  handler: async (ctx, { ingestSignature, ...record }) => {
    ...
    if (record.recipientCount !== undefined && (!Number.isInteger(record.recipientCount) || record.recipientCount < 0)) {
    ...
    const id = await ctx.db.insert('resendWebhookEvents', {
      ...record,
      receivedAt: Date.now()
    })
