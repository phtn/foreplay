...
export function useImageConverter() {
  ...
  const convertOnMainThread = useCallback(
    async (file: File, options: ConversionOptions): Promise<Pick<ConversionResult, 'blob' | 'size' | 'format'>> => {
  ...
  const convert = useCallback(
    async (file: File, options: ConversionOptions): Promise<ConversionResult> => {
      ...
      const workerResult = await new Promise<Pick<ConversionResult, 'blob' | 'size' | 'format'>>((resolve, reject) => {
        ...
        const handleMessage = (event: MessageEvent<unknown>) => {
          const data = event.data as WorkerConversionResponse | WorkerErrorResponse | undefined
        ...
        console.warn('Image worker conversion failed. Falling back to main thread conversion.', workerError)
  ...
  const validateImageFile = useCallback(async (file: File): Promise<string | null> => {
    // Validate file type
    ...
    try {
      ...
      console.log((error instanceof Error ? error.message : '') + 'Valid image formats: jpeg, png, webp, avif, bitmap')
