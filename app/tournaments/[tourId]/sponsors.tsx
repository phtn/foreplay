...
export function createSponsorSlides(sponsors: readonly TournamentSponsor[]) {
  ...
  return Array.from(
    { length: Math.ceil(activeSponsors.length / SPONSORS_PER_SLIDE) },
    (_, slideIndex) =>
      Array.from(
        { length: SPONSORS_PER_SLIDE },
        (_, sponsorIndex) =>
          activeSponsors[
            (slideIndex * SPONSORS_PER_SLIDE + sponsorIndex) %
...
function getSponsorHref(url: string | undefined) {
  ...
  try {
    ...
    return sponsorUrl.protocol === 'http:' || sponsorUrl.protocol === 'https:'
      ? sponsorUrl.toString()
      : undefined
  } catch {
    return undefined
...
function SponsorTile({ sponsor }: { sponsor: TournamentSponsor }) {
  ...
  const content = (
    <>
      <p className='font-ios text-[9px] uppercase tracking-[0.24em] text-white/45 sm:text-[10px]'>
        {sponsor.label ?? 'Official sponsor'}
      </p>
      <p className='mt-3 wrap-break-word font-poly text-base leading-tight text-white sm:text-xl md:text-2xl'>
        {sponsor.value}
      </p>
      {href ? (
        <span className='mt-4 inline-flex items-center gap-1 font-ios text-[9px] uppercase tracking-[0.18em] text-emerald-300/80'>
          Visit partner
  ...
  const className =
    'flex h-full min-w-0 flex-col justify-center rounded-xl border border-white/10 bg-white/5 px-3 py-5 text-center sm:px-6 sm:py-7'
...
export function Sponsors({ sponsors }: SponsorsProps) {
  ...
  const activeSlideIndex = slideCount
    ? carouselState.slideIndex % slideCount
    : 0
  ...
  return (
    <section
      ...
      className='mt-12 overflow-hidden rounded-[1.5rem] border border-slate-800 bg-slate-950 text-white shadow-sm sm:mt-16'
      ...
      <div className='flex items-end justify-between gap-4 border-b border-white/10 px-4 py-5 sm:px-7 sm:py-6'>
        <div>
          <p className='font-ios text-[10px] uppercase tracking-[0.28em] text-emerald-300/70'>
            Supported by
          </p>
          <h2
            id='sponsors-heading'
            className='mt-2 font-poly text-2xl tracking-[-0.02em] sm:text-3xl'>
            Tournament partners
          </h2>
        </div>
        ...
        {slideCount > 1 ? (
          <div className='flex shrink-0 items-center gap-2'>
            <button
              ...
              className='inline-flex size-9 items-center justify-center rounded-full border border-white/15 text-white/70 transition-colors hover:border-white/30 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300/70'
              onClick={() =>
                showSlide(
            ...
            <button
              ...
              className='inline-flex size-9 items-center justify-center rounded-full border border-white/15 text-white/70 transition-colors hover:border-white/30 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300/70'
              onClick={() =>
                showSlide((activeSlideIndex + 1) % slideCount, 1)
              }>
      ...
      <div className='overflow-hidden p-3 sm:p-5'>
        <AnimatePresence
          ...
          <motion.ul
            ...
            className={cn(
              'grid min-h-36 gap-2 sm:min-h-44 sm:gap-4',
              activeSlide.length === 1
                ? 'grid-cols-1'
                : activeSlide.length === 2
                  ? 'grid-cols-2'
                  : 'grid-cols-3'
            )}>
            {activeSlide.map((sponsor, sponsorIndex) => (
              <li
                key={`${activeSlideIndex}-${sponsorIndex}-${sponsor.value}`}
                className='min-w-0'>
      ...
      {slideCount > 1 ? (
        <div
          aria-label='Choose a sponsor group'
          ...
          {slides.map((_, slideIndex) => (
            <button
              ...
              aria-current={
                slideIndex === activeSlideIndex ? 'true' : undefined
              }
              className={cn(
              ...
              onClick={() =>
                showSlide(
                  slideIndex,
                  slideIndex >= activeSlideIndex ? 1 : -1
                )
              }
            />
          ))}
