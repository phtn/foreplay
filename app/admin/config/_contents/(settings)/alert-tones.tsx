...
export const AlertTones = () => {
  ...
  const config = useMemo(() => normalizeAdminAlertsConfig(alertsSetting?.value), [alertsSetting?.value])
  ...
  const normalizedDraftConfig = useMemo<AdminAlertsConfig>(() => {
    return normalizeAdminAlertsConfig({
      ...
      orders: {
        ...
        noteDurationMs: parseNumberInput(drafts.orders.noteDurationMs, config.orders.noteDurationMs),
        gapMs: parseNumberInput(drafts.orders.gapMs, config.orders.gapMs),
        volumeDb: parseNumberInput(drafts.orders.volumeDb, config.orders.volumeDb)
      },
      payments: {
        ...
        noteDurationMs: parseNumberInput(drafts.payments.noteDurationMs, config.payments.noteDurationMs),
        gapMs: parseNumberInput(drafts.payments.gapMs, config.payments.gapMs),
        volumeDb: parseNumberInput(drafts.payments.volumeDb, config.payments.volumeDb)
      },
      signups: {
        ...
        noteDurationMs: parseNumberInput(drafts.signups.noteDurationMs, config.signups.noteDurationMs),
        gapMs: parseNumberInput(drafts.signups.gapMs, config.signups.gapMs),
        volumeDb: parseNumberInput(drafts.signups.volumeDb, config.signups.volumeDb)
      },
      messages: {
        ...
        noteDurationMs: parseNumberInput(drafts.messages.noteDurationMs, config.messages.noteDurationMs),
        gapMs: parseNumberInput(drafts.messages.gapMs, config.messages.gapMs),
        volumeDb: parseNumberInput(drafts.messages.volumeDb, config.messages.volumeDb)
      }
    })
  }, [
  ...
  return (
    <div className='flex h-[92vh] min-w-0 w-full max-w-full flex-col gap-4 overflow-y-scroll pb-24'>
      <div className='flex flex-wrap items-center gap-3'>
        <span>Alerts Tones</span>
        <Toggle title='Enable Audio' checked={isEnabled} onChange={() => setIsEnabled(!isEnabled)} />
      </div>
      ...
      <div className='grid gap-4 xl:grid-cols-4'>
        {ADMIN_ALERT_EVENT_KEYS.map((key) => {
          ...
          return (
            <Card key={key} className='rounded-lg border border-slate-500/60 bg-slate-300 dark:bg-dark-table'>
              <CardContent className='flex flex-col gap-4 p-0'>
                ...
                <Input
                  ...
                  onChange={(e) => setDraftField(key, 'notesInput', e.target.value)}
                ...
                <Select
                  ...
                  onChange={(next) => {
                    if (typeof next === 'string') {
                      setDraftField(key, 'synthType', next as AdminAlertEventConfig['synthType'])
                    }
                  }}
                ...
                {draft.synthType === 'basic' && (
                  <Select
                    ...
                    onChange={(next) => {
                      if (typeof next === 'string') {
                        setDraftField(key, 'waveform', next as AdminAlertEventConfig['waveform'])
                      }
                    }}
                ...
                <div className='grid grid-cols-1 gap-3 sm:grid-cols-3'>
                  <Input
                    ...
                    onChange={(e) => setDraftField(key, 'noteDurationMs', e.target.value)}
                  />
                  <Input
                    type='number'
                    label='Gap ms'
                    value={draft.gapMs}
                    onChange={(e) => setDraftField(key, 'gapMs', e.target.value)}
                  />
                  <Input
                    type='number'
                    label='Volume dB'
                    value={draft.volumeDb}
                    onChange={(e) => setDraftField(key, 'volumeDb', e.target.value)}
                  />
                </div>
                ...
                <Button
                  ...
                  disabled={!isEnabled || !draft.enabled}
