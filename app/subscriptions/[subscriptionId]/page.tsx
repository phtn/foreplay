...
const Page = async ({ params }: PageProps) => {
  ...
  const subscription = await fetchQuery(api.subscriptions.q.getByIdForUser, {
      ...
      <main className='space-y-8'>
        <div className='flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between'>
          <div className='space-y-8'>
            ...
            <div className='space-y-1'>
              <p className='font-ios text-xs uppercase tracking-widest text-sky-500'>
                {subscription.txn_ref_no ?? subscription.form_id}
              </p>
              <h1 className='font-heading text-3xl font-bold tracking-tight'>{subscription.tournament_name}</h1>
              <p className='text-sm text-muted-foreground'>Created {formatCreatedAt(subscription._creationTime)}</p>
            </div>
          </div>
          <span
