...
export type Tier = 'Strong candidate' | 'Possible candidate' | 'Low priority' | 'Not a fit'
...
type CallableDeclaration = TypeScript.ArrowFunction | TypeScript.FunctionDeclaration | TypeScript.FunctionExpression
...
const DISCRIMINATOR_NAMES: ReadonlySet<string> = new Set(['_tag', 'kind', 'state', 'status', 'tag', 'type', 'variant'])
...
function requireOptionValue(argv: readonly string[], index: number, option: string, allowDash = false): string {
...
function loadAliasesFromTsconfig(configPath: string): readonly AliasPattern[] {
  try {
    ...
    return Object.keys(parsed.compilerOptions?.paths ?? {}).map((key) =>
      key.endsWith('/*') ? { exact: null, prefix: key.slice(0, -1) } : { exact: key, prefix: null }
    )
  } catch {
    return []
  }
...
function resolveEngine(choice: EngineChoice, targets: readonly string[]): EngineResolution {
  if (choice === 'lexical') {
    return {
      engine: 'lexical',
      typescript: null,
...
export function analyzeSource(filePath: string, source: string, options: AnalyzeOptions): AnalyzeResult {
  const typescript = options.engine === 'lexical' ? null : (options.typescript ?? tryLoadLocalTypeScript(filePath))
...
function collectTypeScriptFacts(ts: TypeScriptApi, filePath: string, source: string): SourceFacts {
  ...
  const visit = (node: TypeScript.Node): void => {
    ...
    if (ts.isImportDeclaration(node) && ts.isStringLiteral(node.moduleSpecifier)) {
      ...
      const typeOnly = Boolean(clause?.isTypeOnly || (allNamedTypeOnly && !clause?.name))
    ...
    if (ts.isTypeAliasDeclaration(node)) {
      ...
      if (ts.isUnionTypeNode(node.type)) {
        if (node.type.types.length >= 2 && node.type.types.every((member) => ts.isLiteralTypeNode(member))) {
    ...
    if (ts.isSwitchStatement(node)) {
      ...
      if (caseCount >= 2) {
        if (
          (ts.isPropertyAccessExpression(node.expression) && DISCRIMINATOR_NAMES.has(node.expression.name.text)) ||
    ...
    if (ts.isTypeReferenceNode(node)) {
      ...
      facts.immutableTypeHints += 1
    } else if (ts.isAsExpression(node) && node.type.kind === ts.SyntaxKind.ConstKeyword) {
      facts.immutableTypeHints += 1
    }
    ...
    if (
      ...
      if (root && ['Date', 'Intl'].includes(root.text) && !locallyDeclaredGlobals.has(root.text)) {
...
function collectLocallyDeclaredNames(ts: TypeScriptApi, sourceFile: TypeScript.SourceFile): ReadonlySet<string> {
...
function collectLocalPrimitiveBindings(ts: TypeScriptApi, sourceFile: TypeScript.SourceFile): LocalPrimitiveBindings {
    ...
    statement.declarationList.declarations.forEach((declaration) => {
      ...
      if (isDefinitelyPrimitiveExpression(ts, initializer, values)) {
        ...
        initializer.elements.every(
          (element) => ts.isExpression(element) && isDefinitelyPrimitiveExpression(ts, element, values)
        )
      ) {
...
function isDefinitelyPrimitiveExpression(
  ...
  return (
    ...
    (ts.isPrefixUnaryExpression(candidate) &&
      (candidate.operator === ts.SyntaxKind.MinusToken || candidate.operator === ts.SyntaxKind.PlusToken) &&
...
function hasAsyncModifier(ts: TypeScriptApi, node: TypeScript.Node): boolean {
  return Boolean(
    ts.canHaveModifiers(node) && ts.getModifiers(node)?.some((modifier) => modifier.kind === ts.SyntaxKind.AsyncKeyword)
  )
}

function propertyNameText(ts: TypeScriptApi, name: TypeScript.PropertyName): string | null {
  ...
  const tagSets = union.types.map((member) => {
    ...
    member.members.forEach((candidate) => {
      if (
        ...
        if (name && DISCRIMINATOR_NAMES.has(name) && (ts.isStringLiteral(literal) || ts.isNumericLiteral(literal))) {
  ...
  const parameterNames = new Set(
    parameters
      .filter(
        (parameter) =>
          !(
            isDefinitelyPrimitiveType(ts, parameter.type, localTypes, localPrimitiveBindings, new Set<string>()) ||
...
function isDefinitelyPrimitiveType(
  ...
  if (ts.isUnionTypeNode(type)) {
    return type.types.every((member) =>
      isDefinitelyPrimitiveType(ts, member, localTypes, localPrimitiveBindings, seenTypes)
    )
  }
  const indexedObjectType =
    ts.isIndexedAccessTypeNode(type) && ts.isParenthesizedTypeNode(type.objectType)
  ...
  if (ts.isTypeReferenceNode(type)) {
    ...
    if (declaration && ts.isTypeAliasDeclaration(declaration) && !seenTypes.has(typeName)) {
...
function unwrapExpression(ts: TypeScriptApi, candidate: TypeScript.Expression): TypeScript.Expression {
...
function inspectBoundaryType(
  ...
  const visit = (node: TypeScript.Node): void => {
    if (
      ...
      (ts.isTypeReferenceNode(node) && ['Array', 'ReadonlyArray'].includes(node.typeName.getText()))
    ) {
      facts.arrayBoundary += 1
    } else if (ts.isFunctionTypeNode(node)) {
      facts.callbackBoundary += 1
    } else if (ts.isTypeReferenceNode(node) && node.typeName.getText().endsWith('Promise')) {
      facts.promiseBoundary += 1
    } else if (
      node.kind === ts.SyntaxKind.NumberKeyword &&
      !(ts.isIndexedAccessTypeNode(node.parent) && node.parent.indexType === node)
    ) {
      facts.numberBoundary += 1
    } else if (ts.isTypeLiteralNode(node) || ts.isMappedTypeNode(node) || node.kind === ts.SyntaxKind.ObjectKeyword) {
      facts.structuralObjectBoundary += 1
    }
...
function isNullishTypeNode(ts: TypeScriptApi, type: TypeScript.TypeNode): boolean {
  return (
...
function inspectCallLike(
  ...
  if (ts.isIdentifier(expression)) {
    ...
    if (ts.isNewExpression(node) && ['WeakMap', 'WeakSet'].includes(expression.text)) {
      facts.hostRuntimeUses += 1
    }
    if (
      [
    ...
    if (directIoMethod || databaseMethod || ['axios', 'firebase', 'prisma'].includes(root)) {
      facts.networkOrDatabaseCalls += 1
    }
    if (
...
function rootIdentifier(ts: TypeScriptApi, expression: TypeScript.Expression): TypeScript.Identifier | null {
...
function isNestedAccessExpression(
  ...
  return (
    (ts.isPropertyAccessExpression(parent) || ts.isElementAccessExpression(parent)) && parent.expression === expression
  )
}
...
function collectLexicalFacts(filePath: string, source: string): SourceFacts {
  ...
  facts.resultShapes =
    countMatches(noComments, /\b(?:Result|Option|Maybe)\s*</g) + countMatches(code, /\bok\s*:\s*(?:true|false)\b/g)
...
function extractLexicalFunctionNames(source: string): readonly { readonly name: string; readonly exported: boolean }[] {
...
function scoreFacts(
  ...
  const externalImports = runtimeImports.filter(
    (reference) => !isLocalSpecifier(reference.specifier, context) && !isNodeBuiltin(reference.specifier)
  )
  ...
  const frameworkImports = externalImports.filter((reference) =>
    FRAMEWORK_PACKAGES.some(
      (packageName) => reference.specifier === packageName || reference.specifier.startsWith(`${packageName}/`)
    )
  )
  ...
  const identityCoupledApi =
    facts.identitySensitiveReturns > 0 && facts.callbackBoundary > 0 && facts.externalTypeBoundary > 0
  ...
  if (!substantive) {
    ...
    const wholeFileScore = Math.min(
      95,
      clampScore(Math.round(languageFit * 0.8 + migrationValue * 0.2 + 10 - boundaryCost * 0.5))
    )
    const extractionOpportunityScore =
      ...
      !identityCoupledApi
        ? clampScore(
            Math.round(languageFit * 0.6 + migrationValue * 0.1 - Math.min(5, sumContributions(negativeSignals) * 0.05))
          )
        : 0
  ...
  if (facts.generatedGleamInterop > 0) {
    ...
    verdict = sumContributions(negativeSignals) >= 20 ? 'Port with redesign' : 'Whole-file candidate'
  ...
  if (facts.structuralObjectBoundary > 0 || facts.methodObjectBoundary > 0 || facts.externalTypeBoundary > 0) {
    reviewFlags.push(
...
function signal(label: string, weight: number, cap: number, count: number): SignalDefinition {
...
function uniqueRuntimeImports(imports: readonly ImportReference[]): readonly ImportReference[] {
  const unique = new Map<string, ImportReference>()
  imports.filter((reference) => !reference.typeOnly).forEach((reference) => unique.set(reference.specifier, reference))
