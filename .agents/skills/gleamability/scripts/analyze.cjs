...
function resolveEngine(choice, targets) {
    if (choice === "lexical") {
        return {
            engine: "lexical",
            typescript: null,
    ...
    for (const directory of candidateDirectories) {
        try {
            const localRequire = (0, node_module_1.createRequire)(path.join(directory, "__gleamability__.cjs"));
    ...
    if (choice === "typescript") {
        throw new UsageError("The TypeScript engine was requested, but the target project does not provide the typescript package.");
    }
    return {
        engine: "lexical",
        typescript: null,
...
function tryLoadLocalTypeScript(filePath) {
    try {
        return (0, node_module_1.createRequire)(path.join(path.dirname(path.resolve(filePath)), "__gleamability__.cjs"))("typescript");
    }
    catch {
        return null;
    }
